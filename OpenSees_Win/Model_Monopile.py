...
def Monopile_Build(Nodes, Elements, Sections, Aero_Twr, Load_Path, Result_Path, MYS, WF, WS):
    ...
    op.load(int(Nodes_Rot[0]), *DOF_Load_x)
    op.pattern('Plain', 102, 102)  # Rotor
    op.load(int(Nodes_Rot[0]), *DOF_Load_y)
    ...
    op.pattern('Plain', 301, 301)  # Foundation
    op.load(int(Nodes_Fod[0]), *DOF_Load_x)
    op.pattern('Plain', 302, 302)  # Foundation
    op.load(int(Nodes_Fod[0]), *DOF_Load_y)
