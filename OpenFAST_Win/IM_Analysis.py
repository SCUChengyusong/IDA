...
def IM_Analysis(num, File_OpenFAST, Name, Path_OpenFAST, FilePath, OpenFAST_exe, TurbSim_exe, File_TurbSim, Path_TurbSim, TurbSim_Str, TurbSim_Val,
    ...
    with open(Path_OpenFAST + '/{}mps_{}_{}_out.txt'.format(WindSpeed, num, Name), 'a+', encoding='UTF-8') as out:
        subprocess.run([Path_OpenFAST + "/{}".format(TurbSim_exe), Path_TurbSim + '/{}mps_{}_{}'.format(WindSpeed, num, File_TurbSim)], stdout=subprocess.PIPE)
        subprocess.run([Path_OpenFAST + '/{}'.format(OpenFAST_exe), Path_OpenFAST + '/{}mps_{}_{}'.format(WindSpeed, num, File_OpenFAST)], stdout=out)
    out.close()

    shutil.copyfile(Path_OpenFAST + '/{}mps_{}_{}.out'.format(WindSpeed, num, Name), FilePath + '/{}_data.txt'.format(str(num)))
    shutil.copyfile(Path_OpenFAST + '/{}mps_{}_{}.sum'.format(WindSpeed, num, Name), FilePath + '/{}_sum.txt'.format(str(num)))
    shutil.copyfile(Path_OpenFAST + '/{}mps_{}_{}_out.txt'.format(WindSpeed, num, Name), FilePath + '/{}_out.txt'.format(str(num)))
